...
namespace AAk.Windows.Controls
{
    public partial class MonthView : System.Windows.Controls.UserControl
    {
        ...
        public AAk.Utils.PersianDate SelectedDateTime
        {
            get
            {
                return (AAk.Utils.PersianDate)base.GetValue(SelectedDateTimeProperty);
            }
            set
            {
        ...
        private void IncreasePersianMonth(ref int year, ref int month, int number)
        {
            month += number;
            if (month > 12)
            {
        ...
        private void DecreasePersianMonth(ref int year, ref int month, int number)
        {
            month -= number;
            if (month < 1)
            {
        ...
        private void IncreasePersianDay(ref int year, ref int month, ref int day, int number)
        {
            int tempDay = day;
            tempDay += number;
            //شش ماه اول سال
        ...
        private void DecreasePersianDay(ref int year, ref int month, ref int day, int number)
        {
            int tempDay = day;
        ...
        private int CalculatePersianSpan(string weekday)
        {
            switch (weekday)
            {
                case "شنبه":
        ...
        private void previousMonth_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            DecreasePersianMonth(ref yearForNavigating, ref monthForNavigating, 1);
            CalculateMonth(yearForNavigating, monthForNavigating);
        }

        private void previousYear_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            this.yearForNavigating--;
            CalculateMonth(yearForNavigating, monthForNavigating);
        }

        private void nextYear_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            this.yearForNavigating++;
            CalculateMonth(yearForNavigating, monthForNavigating);
        }

        private void goToToday_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            currentYear = AAk.Utils.PersianDate.Now.Year;
            currentMonth = AAk.Utils.PersianDate.Now.Month;
            currentDay = AAk.Utils.PersianDate.Now.Day;
